...
def current_month_spend_by_category(df):
    """Expense totals per category for the current calendar month only (for budget tracking)."""
    now = pd.Timestamp.now().to_period("M")
    df = df.copy()
    df["month"] = df["date"].dt.to_period("M")
    this_month = df[(df["month"] == now) & (df["type"] == "expense")]
    return this_month.groupby("category")["amount"].sum()
